    ...
    if (pickedDate != null) {
      ...
      if (pickedTime != null) {
        ...
        if (scheduledTime.isAfter(now)) {
          Provider.of<BookComicProvider>(context, listen: false)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        // Removido o bloco 'actions' relacionado ao Firebase Auth
        /*
        actions: [
          Consumer<BookComicProvider>(
            builder: (context, provider, child) {
              return TextButton.icon(
                ...
                icon: Icon(
                  provider.currentUser == null ? Icons.person_add_alt_1 : Icons.person_off,
                  color: Colors.white,
                ),
                label: Text(
                  provider.currentUser == null ? 'Login Anônimo' : 'Logout',
          ...
          IconButton(
            ...
            onPressed: () {
              showDialog(
                context: context,
                builder: (context) {
                  final provider = Provider.of<BookComicProvider>(context);
                  return AlertDialog(
                    backgroundColor: Theme.of(context).cardColor,
                    title: Text('Informações do Usuário', style: Theme.of(context).textTheme.headlineMedium!.copyWith(color: Theme.of(context).hintColor)),
                    content: Text(
                      provider.currentUser != null
                          ? 'ID do Usuário: ${provider.currentUser!.uid}\nStatus: Logado (Anônimo)'
                          : 'Status: Deslogado',
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Buscar em sua coleção...', // Ajuste do hintText
              ...
              TabBar(
                ...
                indicatorColor: Theme.of(context).tabBarTheme.indicator!.borderSide.color,
                indicatorWeight: Theme.of(context).tabBarTheme.indicator!.borderSide.width,
              ),
            ],
          ),
        ),
  ...
  Widget _buildReadingList(BuildContext context) {
    return Consumer<BookComicProvider>(
      builder: (context, provider, child) {
        ...
        if (readingItems.isEmpty && provider.bookComics.isEmpty) { // Ajuste para quando a lista está vazia desde o início
          return Center(
            child: Padding(
           ...
           return Center( // Se tem itens, mas a busca não encontrou
            child: Padding(
              padding: const EdgeInsets.all(24.0),
              child: Text(
  ...
  Widget _buildWishlist(BuildContext context) {
    return Consumer<BookComicProvider>(
      builder: (context, provider, child) {
        ...
        if (wishlistItems.isEmpty && provider.bookComics.isEmpty) { // Ajuste para quando a lista está vazia desde o início
          return Center(
            child: Padding(
              padding: const EdgeInsets.all(24.0),
              child: Text(
